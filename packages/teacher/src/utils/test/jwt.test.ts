...
describe('auth utils', () => {
  ...
  describe('decodedToken', () => {
    ...
    it('should throw an error when jwt.decode fails', async () => {
      const mockToken = 'mockToken';
      const mockError = new BaseCustomError("Can't Decode token!", StatusCode.NOT_ACCEPTABLE);

      (jwt.decode as jest.Mock).mockImplementation(() => {
        throw mockError;
      });

      await expect(decodedToken(mockToken)).rejects.toThrow("Can't Decode token!");
    });
  });
});
